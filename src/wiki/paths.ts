...
export function wikiPaths(cwd: string): WikiPaths {
  return {
    ...
    // paths — one of the three §6-sanctioned citation link forms
    ...
    // binaries) — tolerated because consumers ignore non-`.md` files (§11).
    ...
    // documents per OKF §3.1 and the bundle stays conformant (§11) without
    ...
    // bundle-relative path and therefore an OKF §6-sanctioned citation form.
    ...
    // bundle stays conformant (§11) — which is also what keeps them out of
